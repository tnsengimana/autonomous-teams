...
export default function EditAideAgentPage() {
  ...
  useEffect(() => {
    const fetchAgent = async () => {
      try {
        const response = await fetch(`/api/aides/${aideId}/agents/${agentId}`);
        if (!response.ok) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      router.push(`/aides/${aideId}/agents/${agentId}`);
      router.refresh();
    } catch (err) {
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-6">
      <div>
        <Link
          href={`/aides/${aideId}/agents/${agentId}`}
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Modify how this agent appears and behaves within the aide
            </CardDescription>
          </CardHeader>
        ...
        <div className="flex gap-4">
          ...
          <Link href={`/aides/${aideId}/agents/${agentId}`}>
