...
export default function NewAideSubordinatePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`/api/aides/${aideId}/agents`, {
        method: "POST",
        headers: {
      ...
      router.push(`/aides/${aideId}/agents`);
    } catch (err) {
    ...
    <div className="mx-auto max-w-2xl space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">
          Create a new subordinate agent to handle specific tasks for your aide
        </p>
      </div>
